#!/usr/bin/env python3
"""fact-attestation-oracle -- a deterministic attestation-integrity decider for the `fact` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `fact` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='fact', a non-empty
...
def decide(c):
    if c.get("domain") != "fact": return "INVALID"
...
def _run_probes(path):
    ...
    print("fact attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
